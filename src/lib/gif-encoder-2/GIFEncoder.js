...
class GIFEncoder extends EventEmitter {
  ...
  analyzePixels() {
    ...
    if (this.useOptimizer && this.prevImage) {
      ...
      const nPixTotal = data.length / 4
      const match = 100 - Math.ceil((delta / nPixTotal) * 100)
    ...
    if (this.transparent !== null) {
      ...
      const nPixCount = this.indexedPixels.length
      for (var pixelIndex = 0; pixelIndex < nPixCount; pixelIndex++) {
