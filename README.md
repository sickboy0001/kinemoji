...
- [x] 削除機能 (運用の現実解)
  - [x] 「ログイン機能を作らない」という前提の場合、問題のある投稿を消すための手段を確保する必要があります。
  - [x] * **通報ボタン（オプション）:** * ユーザーが「不適切な投稿」を通報できるボタンを作り、自分のDiscordやSlackに通知が飛ぶようにしておくと、即座に対応できます。
...
- [x] スタートページのデザイン
  - [x] スタート画面の再設計
- [x] OG対応　Xとの連携用→Xの癖強いので、ひとまず補修で
  - [x] OG用の設定をNextで可能かの確認、それ用のGIFは有効、プレビュー画面
  - [x] OG：左下にはアプリ名
  - [x] POSTの中身はテキスト＋OGで、テキストは手直ししてもらう想定で。
  - [x] XPostCard kinemoji/docs/20260409_xpostcard/
- [x] 投稿画面のデザイン
- [x] 削除機能
  - [x] 削除ボタンも検討、たぶんデータ一覧に上がらないのみを想定
  - [x] テーブル削除、GIF削除の制限検討
  - [x] GIF含めての削除ボタンも準備→管理者画面？
- [ ] 公開
  - [x] Choitame検討
  - [ ] LB作成
  - [ ] Zenn検討
